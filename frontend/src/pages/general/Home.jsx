...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchFeed = async () => {
      try {
        setError('')
        setLoading(true)
        const res = await axios.get('http://localhost:6969/api/v1/food/get', {
        ...
        if (e.response?.status === 401) {
          setError('Please login to view the feed')
        } else {
  ...
  useEffect(() => {
    // Setup IntersectionObserver after videos are loaded
  ...
  const toggleSave = async (foodId) => {
    ...
    setVideos((prev) => prev.map((v) => {
      ...
      return {
        ...v,
        isSaved: nextSaved,
        saveCount: v.saveCount + (nextSaved ? 1 : -1)
      }
    }))
    try {
      ...
      setVideos((prev) => prev.map((v) => {
        ...
        return {
          ...v,
          isSaved: prevSaved,
          saveCount: v.saveCount + (prevSaved ? 1 : -1)
        }
      }))
    }
  }
